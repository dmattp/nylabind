...
- make db_ handle shared_ptr<> so that database won't get closed out underneath the prepared statements
      ...
      struct Any {
         ...
         static Any* New( const luabind::object& value )
         {
            ...
            if( luabind::type(value) == LUA_TSTRING )
            {
               ...
               unsigned alloc = (sizeof(Any)+len+3) % (unsigned)(~3);
...
//         std::cout << "Create sqlite exec caller for stmt=" << statement << std::endl;
    ...
    public:
        ...
        void do_copied_exec( std::vector<Any*>* copied )
        {
            try {
                ...
                freeParams( copied );
                throw std::runtime_error( std::string("Error Executing PreparedStatement; ") + e.what() );
            } catch( ... ) {
                freeParams( copied );
                throw;
            }
        ...
        void do_bg_exec( std::vector<Any*>* copied, ThreadReporter r )
        {
            do_copied_exec( copied );
        }

        void exec( const luabind::object& params )
        {
            do_copied_exec( toCppFromOptBindingParams(params) );
        }

        StdFunctionCaller
        bgExec( const luabind::object& params )
        {
            return
            StdFunctionCaller
            (  std::bind
                (  &PreparedStatement::do_bg_exec,
                    this,
                    toCppFromOptBindingParams(params),
                    std::placeholders::_1
                )
...
//      .def( "bgExec", &SilkySqlite::bgExec )
      ...
      .def( "bgExec", &PreparedStatement::bgExec )
   ];
